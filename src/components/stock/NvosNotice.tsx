...
export default function NvosNotice({ className = '' }: { className?: string }) {
  return (
    <div
      className={`rounded-[14px] border border-amber-200 bg-amber-50 px-4 py-2.5 text-amber-900 ${className}`}
      role="note"
    >
      <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-700">
        Attention — NL customers
      </p>
      <p className="mt-0.5 text-sm font-semibold">Keuringsstatus</p>
      <p className="mt-1 text-[13px] leading-snug text-amber-800">
        De Piedro EVO schoenen en boots zijn momenteel nog in afwachting van de NVOS-keuring.
      </p>
      <p className="mt-1 text-[13px] leading-snug text-amber-800">
        Zodra de keuring is afgerond, informeren wij u hier direct over.
      </p>
    </div>
  )
}
