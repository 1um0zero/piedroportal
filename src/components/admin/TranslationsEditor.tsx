...
import {
  saveTranslationRows, saveShoeColours,
...
type SectionId = 'construction' | 'closure' | 'type' | 'colour' | 'shoe'
...
export default function TranslationsEditor(props: Props) {
  ...
  // Editable copies, keyed by section.
  const [trans, setTrans] = useState<Record<'construction' | 'closure' | 'type' | 'colour', TransRow[]>>({
  ...
  // Initial snapshots for change detection.
  ...
  const transTab = tab as 'construction' | 'closure' | 'type' | 'colour'
  ...
  // Rows for the active tab, filtered by search.
  const q = search.trim().toLowerCase()
  const visibleTrans = useMemo(
    () => isShoe ? [] : trans[transTab].filter(r =>
      !q || r.key.toLowerCase().includes(q) || [r.en, r.nl, r.fr, r.de].some(v => v.toLowerCase().includes(q))),
  ...
  async function save() {
    ...
    if (isShoe) {
      ...
      const before = new Map(props[transTab].map(r => [r.key, JSON.stringify(r)]))
      const changed = trans[transTab].filter(r => before.get(r.key) !== JSON.stringify(r))
      res = changed.length ? await saveTranslationRows(transTab, changed) : { ok: true }
    }
    setBusy(false)
    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="bg-white rounded-[14px] overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
        ...
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] gap-px bg-stone-100 text-[10px] font-bold uppercase tracking-wider text-stone-400">
          <div className="bg-white px-3 py-2">{isShoe ? t('colour_name') : t('value')}</div>
          ...
          {isShoe ? (
            ...
            visibleTrans.map(r => (
              <div key={r.key} className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] items-center gap-2 px-3 py-1.5">
                <span className="text-sm font-medium text-stone-700 truncate" title={r.key}>{r.key}</span>
                {LANGS.map(l => (
      ...
      <p className="text-xs text-stone-400 mt-3">{t('hint')}</p>
    </div>
  )
}
