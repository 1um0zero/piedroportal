...
import { duplicateOrderAction, deleteOrderAction, cancelOrderAction } from '@/app/actions/orders'
...
export default function OrdersPage({ orders, isAdmin, currentUserId, age = '3m', from, to, showDispatch = false }: Props) {
  ...
  // A Piedro admin may cancel (soft) any other user's order that hasn't entered
  // production yet. Drafts (client-private) and already-cancelled orders are excluded.
  const adminCanCancel = (o: { status?: string; production_state?: string | null }) =>
    isAdmin && !o.production_state && (o.status === 'submitted' || o.status === 'approved')
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={o.id} className="hover:bg-stone-50 transition-colors cursor-pointer"
                    ...
                    {/* Actions — own orders (repeat/delete) + admin cancel on others' orders.
                        The server re-enforces every one of these permissions. */}
                    <td className="px-2 py-3">
                      {!isStock && (
                        (currentUserId && o.user_id === currentUserId) ||
                        (isAdmin && o.user_id !== currentUserId && adminCanCancel(o))
                      ) ? (
                      ...
                      {/* Cancel — Piedro admin, on others' pre-production orders (server re-checks) */}
                      {isAdmin && o.user_id !== currentUserId && adminCanCancel(o) && (
                        <button
