...
## Navigation
- **Gallery** (/gallery) — browse Piedro models by section: **Kids / Men / Women**. On the /gallery route the section switch lives in the page header (hero); **switching section resets all filters**. Six multi-select filters — **Closure, Type, Construction, Colour, Width, Size** — whose available options cascade off whichever other filters are active; plus toggles for **NEW**, **Diabetics** (soft models), **Wishlist-only**, and a catalogue category when arriving from a deep link. Filter labels are per-language, and closure/type/construction/colour **values** are translated from the database. Cards carry a gold **NEW** badge (editorial-new) and a blue **S** badge (diabetics/soft). Hovering a card reveals a tooltip with translated Type and Closure chips, each construction with its widths, and the size range with its scale unit. Click a model to open its detail page.
  - **Exclusive models are NOT behind a reveal toggle.** A signed-in client's **own** exclusive models appear **inline in the normal section grid**, marked with a single gold dot. The only related control is a gold **"My styles"** chip that *isolates* (filters down to) those already-visible exclusives — it never uncovers a hidden collection. Back-office viewers instead see one pastel dot and one filter chip per customer sigla. **Livingstone (LIV) is not a company exclusivity or a toggle** — it is a separate, section-agnostic navbar view with its own hero and Men/Women narrowing chips; plain Livingstone models never appear in the Kids/Men/Women grids.
  - **Search box** matches the model number (style_name) only — not colour or any other field. It is a case-insensitive "contains" match: typing "27" finds any model whose number contains 27 anywhere, NOT only those starting with 27. Use "*" as a wildcard to anchor: "2*" = starts with 2, "*K" = ends with K, "27*9" = starts 27 and ends 9. Do NOT tell the user that typing "2" filters to "starts with 2" — that is false; they must type "2*".
- **Product detail** (/gallery/[id]) — the model's page: image viewer, an **Info / Sizes** table and (when the model has constructions) a **Constructions / Widths** table, available colours, and the order button. The Sizes cell shows the scale unit (EU or UK, defaulting to EU) followed by the size range. Constructions are grouped by shared width sets — each row lists the construction name(s) for that set (translated from the database) with that set's width chips. In **Available Colours**, closure chips appear only when the model has more than one closure; clicking a chip switches the active closure while keeping the current colour (falling back to that closure's first colour). Colour labels are translated from the database and Title-cased. The colour grid shows the variants for the active closure, drawn from the model plus its style/K-suffix siblings, sorted by colour id.
  - **Order button depends on auth state:** a visitor sees **"Log in to order"** (opens an in-place login modal); a logged-in user who cannot order (no company, and not an admin/branch-admin) sees a disabled **"Account pending approval"** pill; an eligible user sees the gold **"Order this model"** link. The order route requires login.
  - **Access:** customer-exclusive and Livingstone detail pages return **404** for anyone outside the owning companies; an exclusive-only client also 404s on general models.
  - The **image viewer** works like this:
    - **Hover** over the main image shows a round magnifier loupe (about 1.8x) that follows the cursor — a localized close-up, the rest of the image unchanged. It appears only while the cursor is over the image and only when not click-zoomed.
    - **Click** the image to zoom *in place* inside the same frame (no pop-up/lightbox): the shoe enlarges and moving the cursor pans it. Clicks step and bounce at the ends: 1st click 2.5x, 2nd click 4x, 3rd click back to 2.5x, 4th click back to normal — then the cycle repeats.
    - A quick cursor flick toward the edge (faster than about 1.2 px/ms) does **not** pan (so you can dart out without the image lurching); only slower, deliberate moves pan. After the cursor leaves and re-enters, panning resumes from the new point without jumping.
    - The zoom level and pan position **persist** while you stay on the page: leaving the image with the cursor keeps it as-is, and switching to another photo or another model/colour keeps the same zoom. The state only resets when you leave the page.
    - When switching model/colour the viewer **keeps the same image index** (e.g. the 4th photo) if that model has it; otherwise it falls back to the first available photo, and shows nothing if the model has no images.
    - **Thumbnail strip** below the image (shown only when the model has more than one photo) lets you pick a specific image; a play/pause button auto-rotates through them at 700ms per photo, and hovering a thumbnail stops the rotation. The gallery holds up to 8 images; any that fail to load are dropped from the strip and main view.
- **Stock / Pair-by-Pair** (/stock) — a gallery-style grid of ready-made stock model cards. A logged-in client clicks one or more models to select them, then the sticky **Place order** bar opens the size/quantity phase for just those models. A visitor sees the cards as non-selectable and gets a login prompt instead; a logged-in user with **no company** may browse and select, but pressing Place order shows a pending-approval message and cannot proceed.
  - **Availability (on hand − reserved).** For each size, available = qty_on_hand − reserved, where reserved is the sum of quantities across **all non-terminal stock orders** (every status except shipped, delivered, cancelled — so submitted, approved **and** in_production all still reserve). The grid shows **only sizes with available > 0**, and only models that still have at least one available size. Each chip is labelled with the **size number**, not a quantity — the available figure is never shown as a number; it acts as a per-size cap (the chip disables at the cap) and the max surfaces only in the chip's hover tooltip.
  - **Adding pairs.** Tapping a size chip adds **one pair** (no +/− controls), capped at that size's available quantity.
  - **What it records.** A stock order carries **no additions and no per-foot/sided configuration at all**. It does capture an **optional Clinician** name and an **optional Patient** name (both stored and shown on the order detail, PDF and emails), optional **Comments**, and a **required Customer Reference** (submission is refused, client- and server-side, if the reference is blank). Do NOT tell a user a stock order has no patient data — it can. The company field is a fixed name for a single-company user, a dropdown for a multi-company user, and all-companies for a Piedro admin.
  - **No draft state.** Stock orders have **no draft stage whatsoever** — they are created directly as **submitted** and reserve their quantities the moment they are placed. Do not tell a user they can hold an unreserved stock draft; none can exist. On submit the server re-checks every line against live availability and rejects the whole order if any quantity exceeds current stock, then sends the client to /orders.
  - **Narrower company gating than the custom flow.** A stock order can be placed **only for a company the user is directly linked to**; **branch-client companies never appear** in the stock picker, even when the same user could order for them elsewhere. Piedro admins are exempt. Likewise, stock **model visibility** uses the user's **own companies' siglas only** (not the branch-augmented set), so /stock is gated more narrowly than the gallery — an exclusive-only client sees only its own exclusive stock models. Never explain a Gallery/Stock mismatch as an availability problem.
  - **After submit.** The order appears in the unified **/orders** list tagged with a **STOCK** chip, showing the first model's name plus an "N models · M pairs" summary; it **never carries a portal order number** (the № cell shows a dash). It generates a PDF and sends notification emails (order-desk, client confirmation, branch copies) — a PDF/email failure is only a warning and never invalidates the order. An expected-dispatch date is computed like custom orders, if the dispatch setting is on.
  - **Detail & lifecycle.** A stock order's detail opens at **/orders/stock/[id]** for clients. The client view is **read-only**: status badge, PDF link, customer card, item table and comments — **no edit, cancel or discard control**. The lifecycle is submitted → approved → in_production → shipped → delivered (or cancelled); there is **no changes_requested** for stock, and **only a Piedro admin can change a stock order's status**. Moving it to a terminal state (shipped/delivered/cancelled) **releases its reservation** back into availability. Stock orders **cannot be repeated or duplicated** — the ↺ repeat action is not offered for stock rows.
- **Catalogues** (/catalogues) — a public page-curl flip-book (no login needed) presenting two books, **Kids** and **Adults**, chosen via a pill tab switcher above the book. **Editions exist only in English and Dutch**, and the language is **auto-selected from the UI locale** — a Dutch UI shows the NL book, every other locale (including FR and DE) shows the EN book. There is **no in-page language picker**; the only in-viewer control switches Kids vs Adults. Do NOT tell a French or German user that an FR/DE catalogue exists or that they can switch the viewer to NL. The book shows a two-page spread on wide screens and a single page on narrow ones, with prev/next arrows, arrow-key navigation, and a page counter; leaves lazy-load as you flip.
...
- **Wishlist** (/wishlist) — favourited models grouped by section (Kids / Men / Women) as product cards, with an empty state linking back to the gallery. **Viewable without login.** Favourites are added from the gallery: the **"Wishlist"** toggle reveals a heart on each card; clicking a card's heart adds/removes it, and once favourites exist an **"N selected"** button can restrict the grid to wishlisted models. The navbar heart shows a badge with the favourite count (capped at 99+), linking to /wishlist. You do **not** order from the wishlist itself — its cards link to the product detail page, and starting an order requires login.
...
- **Repeat order** — in My Orders, click the ↺ icon to duplicate a custom order as a draft (opens the pre-filled form for editing). Stock orders cannot be repeated — the ↺ action is not offered for them.
...
- **Stock** (/admin/stock) — manage stock levels per model/size: on-hand quantities; reserved is computed from every non-terminal stock order — submitted, approved AND in_production all reserve; only shipped/delivered/cancelled release it (available = on hand − reserved).
