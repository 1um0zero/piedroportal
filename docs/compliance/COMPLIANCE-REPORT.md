...
| S4 | 🟠 Med | **No database Row-Level Security (RLS)** as defence-in-depth. The app relies on the service-role key + application checks. The `user_companies` migration did not enable RLS. | Enable RLS on `orders`, `profiles`, `companies`, `user_companies` with policies, even though access is via service role. Defence-in-depth + a common audit expectation. |
...
| S6 | 🟠 Med | **Two unauthenticated API routes:** `/api/orders/send` (service-role, fetches any order, emails PDF — likely dead/duplicate code) and `/api/orders/preview` (renders arbitrary PDF). | Authenticate both, or delete `/api/orders/send` (the live email path is the server action). |
| S7 | 🟡 Low | **Email HTML not escaped** (`patient_name`, `reference`, `full_name` interpolated into email HTML). | HTML-escape all user/patient-supplied values in email templates. |
| S8 | 🟡 Low | **No security headers / CSP.** | Add `Content-Security-Policy`, `Strict-Transport-Security`, `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options` via `next.config` headers. |
