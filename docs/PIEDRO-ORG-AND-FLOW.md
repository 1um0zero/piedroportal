...
| **invoiced** | ⚠️ **em falta** — precisa de `invoice_number` + `invoice_date` |
| **shipped / tracking** | ⚠️ **em falta** — precisa de `tracking_number` + `tracking_url` (cliente quer **link**) |
...
- **`cr56f_order_piedro` → `piedro_order_id`**: o import deve passar a guardar este campo (hoje não
  guarda). ⚠️ confirmar o nome exacto do campo no Dataverse antes do re-import.
